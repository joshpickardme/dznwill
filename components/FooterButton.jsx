...
export default function FooterButton({text = "empty", link = "", special = false, icon = ""}) {
    ...
    if(special) {
        return (
            <button  className={`text-white bg-[var(--black)] flex flex-row gap-3 items-center rounded-md text-2xl md:text-xl pt-2 pb-2 pl-6 pr-3 h-min hover:cursor-pointer`}>{text}{icon}</button>
        )
    } else {
        return (
